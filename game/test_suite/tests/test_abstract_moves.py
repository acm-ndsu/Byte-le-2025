...
class TestAbstractMoves(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.abstract_buff: AbstractBuff = AbstractBuff(buff_amount=1.0)
        self.abstract_debuff: AbstractDebuff = AbstractDebuff(debuff_amount=1.0)
    ...
    def test_base_setters_errors(self):
        ...
        with self.assertRaises(ValueError) as e:
            ...
            self.assertEqual(str(e.exception),
                             ...
                             f'It is a(n) {None.__name__} and has the value of None.')
        with self.assertRaises(ValueError) as e:
        ...
        self.assertEqual(str(e.exception), f'{self.abstract_buff.__class__.__name__}.buff_amount must be a float. '
                                           f'It is a(n) {str.__name__} and has the value of hey.')
        with self.assertRaises(ValueError) as e:
            self.abstract_buff.buff_amount = None
            self.assertEqual(str(e.exception),
                             f'{self.abstract_buff.__class__.__name__}.buff_amount must be a float. '
                             f'It is a(n) {None.__name__} and has the value of None.')

        with self.assertRaises(ValueError) as e:
            self.abstract_debuff.debuff_amount = 'hey'
        self.assertEqual(str(e.exception), f'{self.abstract_debuff.__class__.__name__}.debuff_amount must be a float. '
                                           f'It is a(n) {str.__name__} and has the value of hey.')
        with self.assertRaises(ValueError) as e:
            self.abstract_debuff.debuff_amount = None
            self.assertEqual(str(e.exception),
                             f'{self.abstract_debuff.__class__.__name__}.debuff_amount must be a float. '
                             f'It is a(n) {None.__name__} and has the value of None.')
