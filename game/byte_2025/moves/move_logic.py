...
class MoveLogic:
    """
    This class contains static methods help perform the logic behind each type of move.
    """
    @staticmethod
    def handle_move_logic(user: Character, targets: list[Character], current_move: Move) -> None:
        ...
        match current_move.move_type:
            case MoveType.ATTACK:
                current_move: Attack
                MoveLogic.__calc_and_apply_damage(user, targets, current_move)
            case MoveType.HEAL:
                current_move: Heal
                MoveLogic.__apply_heal_points(targets, current_move)
            case MoveType.BUFF:
                current_move: Buff
                MoveLogic.__handle_stat_modification(targets, current_move)
    ...
    def calculate_modifier_effect(target: Character, current_move: Buff | Debuff) -> int:
        ...
        stat = MoveLogic.__get_stat_object_to_affect(target, current_move)
    ...
    def __calc_and_apply_damage(user: Character, targets: list[Character], current_move: Attack):
        ...
        for target in targets:
            ...
            damage_to_deal: int = MoveLogic.calculate_damage(user, target, current_move)
    ...
    def __apply_heal_points(targets: list[Character], current_move: Heal) -> None:
        ...
        for target in targets:
            ...
            adjusted_healing_amount = MoveLogic.calculate_healing(target, current_move)
